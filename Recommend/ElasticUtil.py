...
class ElasticUtil:
    ...
    def populate_index(self, index_name, gen):
        ...
        self._create_index("ontheblock")
        from elasticsearch.helpers import bulk
        try:
            bulk(self._elastic_client, gen, chunk_size=50)
    ...
    def generate_recommendations(self, vector: list[float], n_recommendations: int):
        ...
        response = self._elastic_client.search(
            ...
            body = {
                ...
                "query": script_query,
                "_source": ["id", "video_name", "video_writer", "song_name", "artist", "instrument", "release_date",  "description"]
            }
        )

        return [x for x in response["hits"]["hits"]]
